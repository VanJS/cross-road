...
export default class PlaneManager {
  ...
  constructor(scene: Phaser.Scene, duck: Duck) {
    ...
    scene.physics.add.overlap(
      this.duck,
      this.planes,
      this.handlePlaneDuckCollision as Phaser.Types.Physics.Arcade.ArcadePhysicsCallback,
      undefined,
      this
    );
  ...
  private setupLanes(): void {
    ...
    for (let i = 0; i < totalPossibleLanes; i += this.laneSpacing) {
      ...
      const speed = Phaser.Math.Between(this.baseSpeed - 30, this.baseSpeed + 30);
    ...
    console.log(`Created ${this.lanes.length} lanes with spacing of ${this.laneSpacing}`);
  }
  ...
  private checkLanesForSpawning(): void {
    ...
    this.lanes.forEach((lane) => {
      ...
      if (currentTime - lane.lastSpawnTime > timeThreshold && Math.random() < this.spawnChance) {
  ...
  private canSpawnInLane(lane: Lane): boolean {
    ...
    // If planes move left, check right side
      ...
      return screenWidth - leftmostPlane.x > this.minDistanceBetweenPlanes;
    }
  }
